...
Polymer({
    ...
    properties: {
        invoiceSettings: {
            type: Array,
            value: () => {
                const settings = config_1.default.get('invoiceSettings');
                return [
    ...
    ready() {
        ...
        const registerFile = config_1.default.get('registerFile');
        /**
        ...
        this.$['select-register-btn'].addEventListener('change', (event) => {
            ...
            if (file) {
                ...
                renderToGrid(grid, register);
                config_1.default.set('registerFile', file.path);
            }
        });
        /**
        ...
        this.$['preview-btn'].addEventListener('click', () => {
            ...
            if (clients.length) {
                electron_1.ipcRenderer.send('invoice-preview', clients[0], this.getInvoiceData());
            }
        });
        /**
        ...
        this.$['save-btn'].addEventListener('click', () => {
            ...
            if (clients.length) {
                const opts = {};
                electron_1.ipcRenderer.send('invoice-save', clients, this.getInvoiceData(), opts);
            }
        });
        /**
        ...
        this.$['save-all-btn'].addEventListener('click', () => {
            ...
            electron_1.ipcRenderer.send('invoice-save', allClients, this.getInvoiceData(), opts);
        });
        /**
     ...
     */
    getInvoiceData() {
        const { laskunumero, maksuehto, viivästyskorko } = config_1.default.get('invoiceSettings');
        return {
    ...
    invoiceSettingsChanged({ path, value }) {
        ...
        this.debounce('invoiceSettingsChanged', () => {
            ...
            config_1.default.set(`invoiceSettings.${pathTable[key]}`, value);
        }, 100);
    }
}); // end Polymer()
/**
